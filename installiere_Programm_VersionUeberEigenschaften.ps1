...
$dateiname = "\Tolle Firma\Supertolles Programm\Programmdatei_mit_richtiger_Version_in_Eigenschaften.dll"
...
if (Test-Path $64bitPath) {
    ...
    $installedVer = [System.Diagnostics.FileVersionInfo]::GetVersionInfo($64bitPath).FileVersion
    ...
    if (Test-Path $32bitPath) {
        ...
        $installedVer = [System.Diagnostics.FileVersionInfo]::GetVersionInfo($32bitPath).FileVersion
        if ($installedVer -lt $desiredProgVer) {
                #Wenn sie kleiner als die Gewünschte ist, geht es weiter.
                $gehtWeiter = 1
            }
